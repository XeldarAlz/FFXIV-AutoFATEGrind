...
internal sealed partial class LogPage
{
    ...
    private const string InspectorDetailId = "##afg_log_detail";
    ...
    private void DrawSelectionBar()
    {
        ...
        Paint.Surface(dl, origin, origin + new Vector2(width, height), Styling.CardRounding * scale,
            Styling.WithAlpha(Styling.AccentViolet, 0.08f), Styling.WithAlpha(Styling.AccentViolet, 0.35f));
        ...
        if (PillButton.Draw("##afg_log_sel_clear", clearLabel, Styling.TextSecondary, PillButton.Emphasis.Ghost, FontAwesomeIcon.Times,
        ...
        if (PillButton.Draw("##afg_log_sel_copy", copyLabel, Styling.AccentViolet, PillButton.Emphasis.Tinted, FontAwesomeIcon.Copy,
                height: Layout.ActionPillHeight))
        {
            CopyRange(low, high);
        }
    ...
    private void DrawLineCard(int index, float height)
    {
        ...
        if (IconButton.Draw(FontAwesomeIcon.Times, "##afg_log_inspect_close", buttonSize, Styling.TextDim, Loc.T(L.Log.Close)))
        {
            ClearSelection();
        }

        ImGui.SetCursorScreenPos(new Vector2(closeX - buttonSize - HeaderGap * scale * 0.5f, headerMidY - buttonSize * 0.5f));
        if (IconButton.Draw(FontAwesomeIcon.Copy, "##afg_log_inspect_copy", buttonSize, Styling.AccentVioletSoft, Loc.T(L.Log.CopyLine)))
        {
    ...
    private float DrawShowing(Vector2 origin)
    {
        ...
        var hit = Hit.Area("##afg_log_reset", resetSize);
        var color = hit.Hovered ? Styling.AccentVioletSoft : Styling.AccentViolet;
