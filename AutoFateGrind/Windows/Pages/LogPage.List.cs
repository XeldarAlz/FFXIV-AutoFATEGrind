...
internal sealed partial class LogPage
{
    ...
    private const string ListId = "##afg_log_list";
    private const string MenuId = "##afg_log_menu";
    ...
    private void DrawRow(ImDrawListPtr dl, int index, Vector2 rowMin, Vector2 rowSize, Columns columns, bool hovered)
    {
        ...
        if (selected)
        {
            Paint.Fill(dl, rowMin, rowMax, Styling.WithAlpha(Styling.AccentViolet, hovered ? 0.22f : 0.16f), rounding);
            Paint.Fill(dl, rowMin, new Vector2(rowMin.X + SelectionBarWidth * scale, rowMax.Y), Styling.AccentViolet, rounding);
        }
        else if (hovered)
        {
    ...
    private void DrawJumpPill(float scrollY, float visibleHeight, float innerWidth)
    {
        ...
        if (PillButton.Draw("##afg_log_jump", label, Styling.AccentViolet, PillButton.Emphasis.Filled, FontAwesomeIcon.ArrowDown, height: Layout.ActionPillHeight))
        {
            follow = true;
            newWhileDetached = 0;
        }
    ...
    private void DrawEmpty()
    {
        ...
        if (bufferedCount > 0)
        {
            ...
            if (PillButton.Draw("##afg_log_reset_empty", Loc.T(L.Log.ResetFilters), Styling.AccentViolet, PillButton.Emphasis.Tinted, FontAwesomeIcon.Undo,
        ...
        var footer = Loc.T(L.Log.Footer, AfgConstants.LogPrefix);
