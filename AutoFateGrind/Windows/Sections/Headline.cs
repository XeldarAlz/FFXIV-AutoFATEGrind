...
internal static class Headline
{
    ...
    public static bool Draw(Configuration cfg, AutoFateController ctrl, RunHistory history)
    {
        ...
        using (Fonts.PushTitle())
            TextDraw.At(TextDraw.Truncate(info.Title + Loc.T(L.Grind.SentenceEnd), maxTextWidth), new Vector2(origin.X, y), Styling.TextStrong);
    ...
    private static (FontAwesomeIcon Icon, Vector4 Color, string Greeting) Greeting() => DateTime.Now.Hour switch
    {
        ...
        >= 17 and < 22 => (FontAwesomeIcon.CloudMoon, Styling.AccentVioletSoft, Loc.T(L.Shell.GreetingEvening)),
    ...
    private static (string Title, string Detail) LastRun(RunHistory history)
    {
        ...
        if (records.Count == 0)
        {
            return (Loc.T(L.Grind.NoRunsYet), Loc.T(L.Grind.StatsAppearHere));
        }

        var record = records[0];
        return (Loc.T(L.Grind.LastRun, record.FatesCompleted), Loc.T(L.Grind.LastRunDetail, Formatting.Elapsed(record.Duration), record.GemstonesEarned));
    }
}
